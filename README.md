...
> **전체를 한 번에 따라가시려면 [00-전체-구축-순서.md](00-전체-구축-순서.md)** 부터 보세요.
> 빈 AWS 계정에서 데모 가능한 상태까지 13단계 체크리스트이고, 각 단계의 배경은
> 아래 번호 문서로 연결됩니다.
...
| 5 | [촬영 대본](05-촬영-대본.md) | 콘솔 조작 순서와 멘트, 파이프라인 시연 구간 | — |
| 6 | [영상 편집 가이드](06-영상-편집-가이드.md) | 샷 리스트, 확대 구간, 자막 ([SRT](video/captions.srt)) | — |
| 7 | [부가 리소스](07-부가-리소스.md) | DB·스토리지·오퍼레이터·IAM — 무엇이 따라오는가 | — |
| 8 | [MLflow 화면 설명](08-mlflow-화면-설명.md) | 데모에서 무엇을 보여주고 무엇을 열지 말 것인가 | — |
| 9 | [콘솔에서 설명하기](09-콘솔에서-설명하기.md) | CLI로 만든 것을 화면 어디서 보여줄 것인가 | — |
| 10 | [데모 순서](10-데모-순서.md) | **반입 · 검증 · 자동화** 3부 — 메뉴와 멘트 | — |
| 11 | [MaaS 데모](11-maas-데모.md) | Models as a Service — 켜는 데 필요했던 숨은 전제 3개 | — |
| 12 | [검증 범위](12-검증-범위.md) | 0~10 사다리 — 플랫폼팀은 0~6, 편향·가드레일은 서비스팀 | — |
| 13 | [모델 공급망 보안](13-모델-공급망-보안.md) | 커뮤니티 원본 vs Red Hat 검증 — 어디서 받아 무엇을 실행하나 | — |

처음부터 끝까지 **약 4시간**이고, 대부분은 기다리는 시간입니다.

검증 파이프라인(11단계), EvalHub 성능 측정(12단계), MLflow 연동(13단계)은
[00-전체-구축-순서.md](00-전체-구축-순서.md)에 있습니다. 그 세 단계를 만들면서 겪은
것은 [2026-09-26 작업 기록](notes/2026-09-26-작업기록.md)에 따로 적었습니다.
...
| 모델 | Qwen3-8B-AWQ (5.7 GiB), Qwen3-32B-AWQ (18.0 GiB), Qwen3.6-35B-A3B-FP8 (35.0 GiB) |
| 평가·추적 | EvalHub (GuideLLM), MLflow 3.14.0, 모델 레지스트리 |
| 검증 파이프라인 | `model-intake-validation` — 배포·스모크·측정·게이트·승격·복원 8단계 |
...
**파이프라인 캐싱은 배포하지 않고도 "성공"합니다.** KFP가 부작용 있는 단계를 캐시로
처리하면 아무것도 하지 않은 채 SUCCEEDED가 됩니다. 후보 모델을 배포하지 않은 채 세
단계가 성공으로 보고됐고, 그 뒤 에러가 모델 문제처럼 보였습니다.
→ [11단계](00-전체-구축-순서.md)

**파이프라인 코드에 한국어를 쓸 수 없습니다.** 스펙을 저장하는 MariaDB 컬럼이 latin1이고,
KFP는 컴포넌트 소스를 스펙에 그대로 박아 넣습니다. API는 "Failed to create a pipeline"만
돌려주고 진짜 원인은 파드 로그에만 있습니다.
→ [11단계](00-전체-구축-순서.md)

**MLflow CRD는 클러스터 범위입니다.** `metadata.namespace`가 무시되고
`redhat-ods-applications`에 배포되므로, 내가 보는 네임스페이스에는 Job도 PVC도 이벤트도
없습니다. 실패 이유가 다른 곳에만 남습니다.
→ [13단계](00-전체-구축-순서.md)
...
| 기록 | 내용 |
|---|---|
| [2026-09-26](notes/2026-09-26-작업기록.md) | MLflow 연동, 성능 측정의 재현성, 검증 파이프라인 구현, 35B 비교 벤치마크 |
